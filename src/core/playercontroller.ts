...
export class PlayerController {
  ...
  constructor(scene: Scene, input: Input) {
    this.input = input;
    this._camera = new FreeCamera("playerCamera", new Vector3(0, Config.movement.groundHeight, 0), scene);
  ...
  private handleLook(): void {
    ...
    this._yaw += this.input.mouseDeltaX * sens;
    this.pitch += this.input.mouseDeltaY * sens * (Config.mouse.invertY ? -1 : 1);
    this.pitch = Math.max(-Math.PI / 2 + 0.01, Math.min(Math.PI / 2 - 0.01, this.pitch));

    const lookDir = new Vector3(
      Math.sin(this._yaw) * Math.cos(this.pitch),
      Math.sin(this.pitch),
      -Math.cos(this._yaw) * Math.cos(this.pitch)
    );

    this._camera.setTarget(this._camera.position.add(lookDir));
  }
